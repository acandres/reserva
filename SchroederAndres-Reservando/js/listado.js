...
Listado.prototype.buscarRestaurante = function(id) {
    ...
    var restaurantExiste = this.restaurantes.filter(function(idRestaurant){
	    return idRestaurant.id === id;
    });
    if (restaurantExiste){
        return restaurantExiste;
    }
         return "No se ha encontrado ningún restaurant";
}
...
//    }
    // hago un map de map y convierto el array de arrays en un solo array
       this.restaurantes.map(function(restaurante){
            return restaurante.horarios.map(function(horario) {
                arregloHorarios.push(horario);
            })  
            //return restaurante.horarios; 
        });
   
    return (this.eliminaRepetidos(arregloHorarios)).sort();
}


Listado.prototype.eliminaRepetidos = function (repetidos) {
    var elementosUnicos = repetidos.filter(function(elem,index,self){
